<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold font-display text-[var(--color-text)]">Purchase Orders</h1>
            <p class="text-sm text-gray-500 mt-1">Manage purchase requests, supplier quotations, and purchase order logs.</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex-1 max-w-md relative">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" placeholder="Search by PO # or Supplier...">
        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($purchases as $p)
                        <tr class="hover:bg-gray-50 transition duration-150">
                            <td class="py-4 px-6 font-mono font-medium text-blue-600">{{ $p->purchase_number }}</td>
                            <td class="py-4 px-6 font-medium text-gray-900">{{ $p->supplier->name }}</td>
                            <td class="py-4 px-6">{{ $p->purchase_date }}</td>
                            <td class="py-4 px-6 text-right font-mono">Rp {{ number_format($p->tax_amount, 0, ',', '.') }}</td>
                            <td class="py-4 px-6 text-right font-mono font-semibold text-gray-900">Rp {{ number_format($p->grand_total, 0, ',', '.') }}</td>
                            ...
                            <td class="py-4 px-6 text-center space-x-2">
                                <button wire:click="edit({{ $p->id }})" class="text-blue-600 hover:text-blue-900 font-medium cursor-pointer">Edit</button>
                                <button wire:click="delete({{ $p->id }})" wire:confirm="Are you sure you want to delete this purchase order?" class="text-red-600 hover:text-red-900 font-medium cursor-pointer">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                    ...
                    <div>
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="purchase_number" readonly class="mt-1 block w-full border border-gray-300 rounded-md bg-gray-50 py-2 px-3 sm:text-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <select wire:model="purchase_status" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 sm:text-sm bg-white">
                                    <option value="draft">Draft</option>
                                    <option value="ordered">Ordered</option>
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" wire:model="purchase_date" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 sm:text-sm">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                                        ...
                                        <th class="pb-2 w-44 text-right">Unit Cost</th>
                                    ...
                                    @foreach($items as $index => $item)
                                        <tr class="py-2">
                                            ...
                                            <td class="py-2 text-center align-middle">
                                                <button type="button" wire:click="removeItem({{ $index }})" class="text-red-600 hover:text-red-900 font-medium cursor-pointer">
                                                    Remove
                                                </button>
                                            </td>
                                        </tr>
