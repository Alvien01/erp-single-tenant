<div class="flex-1 flex w-full h-full">
    ...
    @if(!$currentSession)
    <div class="absolute inset-0 z-50 glass-panel flex flex-col items-center justify-center p-4">
    ...
    <div class="flex-1 flex flex-col bg-slate-900/50 border-r border-slate-800">
        ...
        <div class="h-14 bg-slate-900 border-t border-slate-800 flex items-center justify-between px-4 shrink-0">
            <div class="flex gap-2">
                <button wire:click="$set('showPendingModal', true)" class="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm font-medium transition-colors">
            ...
            <div class="flex items-center gap-4 text-sm text-slate-400">
                ...
                <button wire:click="closeSession" class="text-red-400 hover:text-red-300 font-medium">Close Shift (EOD)</button>
            </div>
        </div>
    </div>

    <!-- RIGHT PANEL: Cart & Checkout -->
    <div class="w-[400px] flex flex-col bg-slate-800 border-l border-slate-700 shrink-0 z-10 shadow-[-10px_0_30px_rgba(0,0,0,0.2)]">
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-1">
            ...
            <div class="bg-slate-900/50 hover:bg-slate-700/50 p-3 rounded-xl group transition-colors">
                <div class="flex justify-between items-start mb-2">
                    <div class="pr-2">
                        <h4 class="text-sm font-medium text-slate-200 line-clamp-1">{{ $item['name'] ?? '' }}</h4>
                        <div class="text-[11px] text-slate-400 font-mono">Rp {{ number_format($item['price'] ?? 0, 0, ',', '.') }} / {{ $item['unit'] ?? '' }}</div>
                    </div>
                    <div class="font-mono font-bold text-slate-100 text-sm">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <button wire:click="removeFromCart('{{ $key }}')" class="p-2 text-slate-500 hover:text-red-400 transition-colors rounded-lg hover:bg-red-500/10 opacity-0 group-hover:opacity-100">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div wire:click="selectMember({{ $m->id }})" class="p-3 bg-slate-900/50 hover:bg-blue-900/30 border border-slate-700 hover:border-blue-500/50 rounded-xl cursor-pointer transition-colors flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-medium text-slate-200 text-sm">{{ $m->name }}</div>
                                    <div class="text-xs text-slate-500">{{ $m->phone ?? '' }} • {{ $m->tier ?? 'regular' }}</div>
                                </div>
                            </div>
                            @empty
                                @if($memberSearch)
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col">
            ...
            <div class="p-6">
                <!-- Total Amount Banner -->
                <div class="bg-blue-900/20 border border-blue-500/30 rounded-xl p-6 text-center mb-6">
                    <div class="text-blue-300 text-sm font-medium mb-1 uppercase tracking-wider">Total Pembayaran</div>
                    <div class="text-4xl font-display font-bold text-white font-mono">
                        Rp {{ number_format($this->grandTotal, 0, ',', '.') }}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-6">
                    <button wire:click="$set('paymentMethod', 'cash')" class="p-4 rounded-xl border flex flex-col items-center gap-2 transition-all {{ $paymentMethod === 'cash' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-500' }}">
                    ...
                    <button wire:click="$set('paymentMethod', 'qris')" class="p-4 rounded-xl border flex flex-col items-center gap-2 transition-all {{ $paymentMethod === 'qris' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-500' }}">
                        <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>
                        <span class="font-medium text-sm">QRIS / e-Wallet</span>
                    </button>
                </div>
                ...
                <div class="space-y-4">
                    <div class="flex justify-center p-4 bg-white rounded-xl">
                        <!-- Placeholder for QR Code image from Midtrans/etc -->
                        <div class="w-48 h-48 bg-gray-200 border-2 border-dashed border-gray-400 flex items-center justify-center text-gray-500 text-sm font-medium text-center">
                            QRIS<br>Generates Here
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-1">No. Referensi (Opsional)</label>
                        <input type="text" wire:model.defer="qrisReference" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-blue-500" placeholder="Input nomor ref jika ada">
                    </div>
                </div>
                @endif
            </div>

            <div class="p-5 border-t border-slate-700 bg-slate-900 flex gap-3">
                <button wire:click="$set('showPaymentModal', false)" class="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl font-medium transition-colors">Batal</button>
                <button wire:click="processPayment" class="flex-1 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-lg shadow-[0_0_20px_rgba(37,99,235,0.3)] transition-all active:scale-[0.98]">
    ...
    <div class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div class="flex flex-col gap-4 max-w-sm w-full">
            <!-- Thermal Receipt Preview -->
            <div class="bg-white text-black p-6 rounded shadow-2xl font-mono text-xs w-full mx-auto print:shadow-none print:p-0" id="receiptArea">
    ...
    @if($showPendingModal)
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
            ...
            <div class="p-6 overflow-y-auto bg-slate-900/50">
                @if(count($cart) > 0)
