<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold font-display text-[var(--color-text)]">Customers</h1>
            <p class="text-sm text-gray-500 mt-1">Manage customer registry and company contacts.</p>
        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($customers as $cust)
                        <tr class="hover:bg-gray-50 transition duration-150">
                            <td class="py-4 px-6 font-medium text-gray-900">{{ $cust->name }}</td>
                            <td class="py-4 px-6 text-gray-700">{{ $cust->company_name ?? '-' }}</td>
                            <td class="py-4 px-6 text-gray-600">{{ $cust->email ?? '-' }}</td>
                            <td class="py-4 px-6 text-gray-600">{{ $cust->phone ?? '-' }}</td>
                            <td class="py-4 px-6 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $cust->type === 'company' ? 'bg-blue-100 text-blue-800' : 'bg-emerald-100 text-emerald-800' }}">
                                    {{ ucfirst($cust->type) }}
                                </span>
                            </td>
                            <td class="py-4 px-6 text-center space-x-2">
                                <button wire:click="edit({{ $cust->id }})" class="text-blue-600 hover:text-blue-900 font-medium cursor-pointer">Edit</button>
                                <button wire:click="delete({{ $cust->id }})" wire:confirm="Are you sure you want to delete this customer?" class="text-red-600 hover:text-red-900 font-medium cursor-pointer">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
