<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold font-display text-[var(--color-text)]">Purchase Requests</h1>
            <p class="text-sm text-gray-500 mt-1">Manage internal purchase requests from various departments.</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <div class="max-w-md relative">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" placeholder="Search by PR number or department...">
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead>
                    <tr class="bg-gray-50 text-left font-semibold text-gray-500">
                        <th class="py-3.5 px-6">PR Number</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($requests as $pr)
                        <tr class="hover:bg-gray-50 transition duration-150">
                            <td class="py-4 px-6 font-mono font-semibold text-blue-600">{{ $pr->pr_number }}</td>
                            <td class="py-4 px-6 text-gray-900 font-medium">{{ $pr->department }}</td>
                            <td class="py-4 px-6 text-gray-700 font-medium">{{ $pr->requester->name }}</td>
                            <td class="py-4 px-6 text-gray-600">{{ $pr->created_at->format('Y-m-d') }}</td>
                            <td class="py-4 px-6 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                    ...
                                    {{ $pr->status === 'canceled' ? 'bg-red-100 text-red-800' : '' }}
                                ">
                                    {{ ucfirst($pr->status) }}
                                </span>
                            </td>
                            <td class="py-4 px-6 text-center space-x-2">
                                @if($pr->status === 'draft')
                                    <button wire:click="edit({{ $pr->id }})" class="text-blue-600 hover:text-blue-900 font-medium cursor-pointer">Edit</button>
                                    <button wire:click="approveRequest({{ $pr->id }})" class="text-emerald-600 hover:text-emerald-900 font-medium cursor-pointer">Approve</button>
                                    <button wire:click="rejectRequest({{ $pr->id }})" class="text-red-600 hover:text-red-900 font-medium cursor-pointer">Reject</button>
                                @else
                ...
                <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full sm:p-6">
                    ...
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900 font-display" id="modal-title">
                            {{ $isEditMode ? 'Edit Purchase Request' : 'Add New Purchase Request' }}
                        </h3>
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="department" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 sm:text-sm" placeholder="e.g. IT, Operations, Marketing">
                        ...
                        <div class="mt-6 border-t border-gray-200 pt-6">
                            <div class="flex items-center justify-between mb-4">
                                <h4 class="text-md font-medium text-gray-900 font-display">Requested Items</h4>
                                ...
                                @foreach($items as $index => $item)
                                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                                        ...
                                        <div class="md:col-span-4">
                                            <label class="block text-xs text-gray-500">Notes / Rationale</label>
                                            <input type="text" wire:model="items.{{ $index }}.notes" class="mt-1 block w-full border border-gray-300 rounded-md py-1.5 px-3 text-sm" placeholder="Reason for request">
                                        </div>
