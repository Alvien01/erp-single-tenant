                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($attendances as $att)
                            <tr class="hover:bg-gray-50 transition duration-150">
                                <td class="py-4 px-6 font-medium text-gray-900">{{ $att->date }}</td>
                                <td class="py-4 px-6 text-gray-700 font-semibold">{{ $att->employee->name }}</td>
                                <td class="py-4 px-6 font-mono text-gray-600">{{ $att->check_in }}</td>
                                <td class="py-4 px-6 font-mono text-gray-600">{{ $att->check_out ?? '--:--' }}</td>
                                <td class="py-4 px-6 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $att->status === 'present' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800' }}">
                                        {{ ucfirst($att->status) }}
                                    </span>
                                </td>
                            </tr>
